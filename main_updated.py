...
class HobbyAllocator:
    ...
    def create_allocation_model(self, premium_activities=None, premium_factor=0.1):
        ...
        model.solve(pulp.PULP_CBC_CMD(msg=True, timeLimit=600))
        
        if model.status != 1:
            print(f"WARNING: Model solution status: {model.status}")
            print(f"The model may not have found an optimal solution.")
            print(f"There may be issues with your constraints or data.")
